 ...
 public:
	...
	Double_t GetActivity(TString v, TString iso) {
		...
		if (v=="CopperScrewPV") {
			...
			else if (chain=="Pb210") act=200.; //was 68
			else if (GetIsotopeSource(iso)=="Radiogenic") {
				act=GetRadiogenicActivity(iso, Texp_CCD, Trun);
			}
			else if (chain=="U238") act=0.63;
			else if (chain=="Ra226") act=0.43;
			else if (chain=="Th232") act=2.0;
			else if (iso=="40a19z") act=0.04;
	...
	TString GetProposalCategory(TString v, TString iso) {
		TString name=GetPartName(v);
		TString chain=GetIsotopeChain(iso);
		if (name=="CCD" && (iso=="210a82z" || iso=="210a83z" || iso=="32a14z" || iso=="32a15z")) {
			return TString("Si32 & Pb210");
		} else if (name=="CCD") {
			return TString("CCD Contamination");
		} else if (GetIsotopeChain(iso)=="Co"){
			return TString("External");
		} else if ((name=="Flex Cable" || name=="Module Screws") && (chain=="U238" || chain=="Ra226" || chain=="Pb210" || chain=="Th232" || iso=="40a19z")) {
			return TString("External");
		}
		else {
