...
//#include "G4EmProcessOptions.hh" //deprecated
...
// Neutron capture - all E
  #include "G4NeutronRadCapture.hh"

//adding neutron thermal scattering
  #include "G4NeutronHPThermalScattering.hh"
  #include "G4NeutronHPThermalScatteringData.hh"

// adding neutron fission     
...
DAMICPhysicsListLivermore::DAMICPhysicsListLivermore() : G4VUserPhysicsList()
{
    ...
    cutForElectron      = 1*micrometer; //original code was 1 nanometer, but changed to 1 micrometer for tracking purposes
    ...
    param->SetLowestElectronEnergy(500*eV);
...
void DAMICPhysicsListLivermore::ConstructEM() 
{
   ...
   man->SetAtomDeexcitation(new G4UAtomicDeexcitation());
    ...
    while((*theParticleIterator)())
    {
        ...
        //G4cout <<  " while : " << particleName << G4endl;
        ...
        else if(particleName == "e-")
        {
            ...
            //eIonisation->SetEmModel(new G4LivermoreIonisationModel()); from v.10.4
    ...
    */
    ...
    while ((*theParticleIterator)())
    {
        ...
        else if(particleName == "neutron") 
        {
            ...
            theElasticNeutronHP->SetMinEnergy(3.9*eV);//modified to include the thermal scattering < 4eV //theHPMin );
            ...
            pmanager->AddDiscreteProcess( theElasticProcess );
	         ...
	         //adding thermal scattering
	    	G4NeutronHPThermalScattering* thermal_model = new G4NeutronHPThermalScattering();
	    	theElasticProcess->RegisterMe( thermal_model );
	    	theElasticProcess->AddDataSet( new  G4NeutronHPThermalScatteringData);
	    	pmanager->AddDiscreteProcess( theElasticProcess );
	    	...
	    	//MS  : add G4ParticleRadCapture <20MeV and RadCapture all E
            //from https://indico.cern.ch/event/776050/contributions/3240672/attachments/1781021/2897454/HadPhys2SaoPaulo.pdf
            G4NeutronRadCapture * theAllENeutronCaptureModel = new G4NeutronRadCapture;
            theAllENeutronCaptureModel->SetMinEnergy(19.9*MeV);
            //theAllENeutronCaptureModel->SetMaxEnergy(19.9*MeV);
            theCaptureProcess->RegisterMe(theAllENeutronCaptureModel);

	    
            G4ParticleHPCapture * theLENeutronCaptureModel = new G4ParticleHPCapture;
            theLENeutronCaptureModel->SetMinEnergy(theHPMin);
            theLENeutronCaptureModel->SetMaxEnergy(19.9*MeV);
			...
			G4NeutronFissionProcess* processnFiss = new G4NeutronFissionProcess();
			pmanager->AddDiscreteProcess(processnFiss);
          
            // cross section data set
			G4NeutronHPFissionData* dataSetnFiss = new G4NeutronHPFissionData();
			processnFiss->AddDataSet(dataSetnFiss);
            //
            // models
			G4NeutronHPFission* modelnFiss = new G4NeutronHPFission();
			processnFiss->RegisterMe(modelnFiss);  
...
void DAMICPhysicsListLivermore::ConstructGeneral() 
{
    ...
    // G4cout << G4endl << "  tests radioactive decay ON " << G4endl ;
...
void DAMICPhysicsListLivermore::SetCuts()
{
    ...
    // XXX Length cuts to different regions are applied at DetectorConstruction (where the GDML is
    // readed) XXX
